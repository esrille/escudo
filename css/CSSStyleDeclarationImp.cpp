...
bool CSSStyleDeclarationImp::resolveBorderConflict(CSSBorderColorValueImp& color, CSSBorderStyleValueImp& style, CSSBorderWidthValueImp& width,
                                                   CSSBorderColorValueImp& underColor, CSSBorderStyleValueImp& underStyle, CSSBorderWidthValueImp& underWidth)
{
    if (underStyle == CSSBorderStyleValueImp::None)
        return false;
    if (underStyle == CSSBorderStyleValueImp::Hidden) {
        style.setValue();
        width.setValue(0.0f, css::CSSPrimitiveValue::CSS_PX);
        return true;
    }
    if (width < underWidth || width == underWidth && style < underStyle) {
        color.specify(underColor);
        style.specify(underStyle);
        width.specify(underWidth);
        return true;
    }
    return false;
}

void CSSStyleDeclarationImp::resolveBorderConflict(CSSStyleDeclarationPtr style, unsigned trbl)
{
    CSSStyleDeclarationImp* imp = style.get();
    if (!imp)
        return;
    if (trbl & 0x1)
        resolveBorderConflict(borderTopColor, borderTopStyle, borderTopWidth,
                            imp->borderTopColor, imp->borderTopStyle, imp->borderTopWidth);
    if (trbl & 0x2)
        resolveBorderConflict(borderRightColor, borderRightStyle, borderRightWidth,
                            imp->borderRightColor, imp->borderRightStyle, imp->borderRightWidth);
    if (trbl & 0x4)
        resolveBorderConflict(borderBottomColor, borderBottomStyle, borderBottomWidth,
                            imp->borderBottomColor, imp->borderBottomStyle, imp->borderBottomWidth);
    if (trbl & 0x8)
        resolveBorderConflict(borderLeftColor, borderLeftStyle, borderLeftWidth,
                            imp->borderLeftColor, imp->borderLeftStyle, imp->borderLeftWidth);
}

void CSSStyleDeclarationImp::resolveLeftBorderConflict(CSSStyleDeclarationPtr style)
{
    CSSStyleDeclarationImp* imp = style.get();
    if (!imp)
        return;
    if (borderLeftStyle == CSSBorderStyleValueImp::Hidden ||
        imp->borderRightWidth < borderLeftWidth ||
        imp->borderRightWidth == borderLeftWidth && imp->borderRightStyle < borderLeftStyle)
    {
        imp->borderRightStyle.setValue();
        imp->borderRightWidth.setValue(0.0f, css::CSSPrimitiveValue::CSS_PX);
    }
    else if (imp->borderRightStyle == CSSBorderStyleValueImp::Hidden ||
             borderLeftWidth < imp->borderRightWidth ||
             borderLeftWidth == imp->borderRightWidth && borderLeftStyle < imp->borderRightStyle)
    {
        borderLeftStyle.setValue();
        borderLeftWidth.setValue(0.0f, css::CSSPrimitiveValue::CSS_PX);
    }
}

void CSSStyleDeclarationImp::resolveTopBorderConflict(CSSStyleDeclarationPtr style)
{
    CSSStyleDeclarationImp* imp = style.get();
    if (!imp)
        return;
