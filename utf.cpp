...
static Folding CommonCaseInMappingOrder[] = // Sorted according to mapping field.
{
    0x0041, 0x0061, // LATIN CAPITAL LETTER A
    0x0042, 0x0062, // LATIN CAPITAL LETTER B
    0x0043, 0x0063, // LATIN CAPITAL LETTER C
...
static FullFolding FullCase[] =
{
    ...
    0x1F56, 0x03C5, 0x0313, 0x0342, // GREEK SMALL LETTER UPSILON WITH PSILI AND PERISPOMENI1F80, F, 1F00 03B9, // GREEK SMALL LETTER ALPHA WITH PSILI AND YPOGEGRAMMENI
...
{
    char32_t utf32 = *(const char32_t*) key;
    char32_t code = ((const Folding*) elm)->code;

    if (utf32 == code)
    {
        return 0;
    }
    return (utf32 < code) ? -1 : 1;
}

static int CompareMapping(const void* key, const void* elm)
{
    char32_t utf32 = *(const char32_t*) key;
    char32_t mapping = ((const Folding*) elm)->mapping;
...
int utf16icmp(const char16_t* a, const char16_t* b)
{
    for (;;)
    {
        char16_t ia = utftolower(*a);
        char16_t ib = utftolower(*b);
        if (ia != ib)
        {
            return (ia < ib) ? -1 : 1;
        }
...
void hexdump(const void* ptr, size_t len)
{
    ...
    for (j = 0; j < len; j += 16) {
        ...
        if (16 < n)
            n = 16;
        printf("%08x: ", (const uint8_t*) ptr + j);
        for (i = 0; i < n; i++)
